...
const TITAN_INVOKE_TIMEOUT_MS = Number(process.env.TITAN_INVOKE_TIMEOUT_MS || '12000');
const TITAN_MAX_ATTEMPTS = Math.max(1, Number(process.env.TITAN_MAX_ATTEMPTS || '2'));
...
async function invokeTitanImageGenerator(
  ...
  for (let attempt = 1; attempt <= TITAN_MAX_ATTEMPTS; attempt++) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), TITAN_INVOKE_TIMEOUT_MS);
    ...
    try {
      ...
      const normalizedError = normalizeTitanError(error);
      lastError = normalizedError;

      const retriable = isRetriableTitanError(normalizedError);
      console.warn(`Titan ${request.taskType} attempt ${attempt}/${TITAN_MAX_ATTEMPTS} failed: ${normalizedError.message} (retriable: ${retriable})`);

      if (!retriable || attempt >= TITAN_MAX_ATTEMPTS) {
  ...
  throw lastError || new Error('Titan invocation failed');
}

function normalizeTitanError(error: any): Error {
  const name = String(error?.name || '');
  const message = String(error?.message || 'Unknown Titan error');
  const aborted = name === 'AbortError' || message.toLowerCase().includes('abort');

  if (aborted) {
    return new Error(`Titan request timeout after ${TITAN_INVOKE_TIMEOUT_MS}ms`);
  }
...
function isRetriableTitanError(error: Error): boolean {
